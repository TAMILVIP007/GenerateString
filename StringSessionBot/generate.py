...
async def generate_session(bot, msg, telethon=False):
    ...
    if telethon:
        ...
        client.storage.SESSION_STRING_FORMAT=">B?256sQ?"
        string_session = await client.export_session_string()
    text = "**{} STRING SESSION** \n\n`{}` \n\nGenerated by @StringRioBot".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Successfully Fetching {} session string. \n\nPlease check the Saved Message!".format("telethon" if telethon else "pyrogram"))


async def cancelled(msg):
    if "/cancel" in msg.text:
        await msg.reply("cancelling Process!", quote=True, reply_markup=InlineKeyboardMarkup(Data.generate_button))
        return True
